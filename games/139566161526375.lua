		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Heartbeat', 'AutoClicker', 0, function()
					if not Utility.Entity.IsAlive(LocalPlayer) then return end
					local Tool = Utility.Entity.Inventory.Character.Get()
			...
			if callback then
				...
				Connection = BridgeDuel.Remotes.KnockBackApplied.OnClientEvent:Connect(function(plr1, dir, pos, mag, data)
					if not LongJump.Enabled then
						...
						if Original then
							task.spawn(Original, plr1, dir, pos, Magnitude, data)
						end
					end
				end)
			else
...
task.defer(function()
	...
	LongJump = Sections.Movement:CreateToggle({
		...
		Callback = function(callback)
			repeat task.wait() until BridgeDuel.Remotes.KnockBackApplied
			if callback then
				if Utility.Entity.Inventory.Backpack.Find('tnt') or Utility.Entity.Inventory.Character.Find('tnt') then
					local v = getconnections(BridgeDuel.Remotes.KnockBackApplied.OnClientEvent)[1]
					if v and v.Function then
...
task.defer(function()
	...
	BedAlarm = Sections.Visual:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Stepped', 'BedAlarm', 0.5, function()
					for _, v in workspace:GetDescendants() do
						...
						for _, v in Players:GetPlayers() do
							...
							if not Utility.Entity.IsAlive(v) then continue end
				...
				Utility.Misc.Events.Remove('Stepped', 'BedAlarm')
				Notified = false
			end
		end
	})
...
task.defer(function()
	...
	BedOverlay = Sections.Visual:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Heartbeat', 'BedOverlay', 0.5, function()
					for _, bed in workspace.Map:GetDescendants() do
						...
						if not Hitbox then Utility.Visual.BillBoard.Remove(bed) continue end
						...
						Utility.Visual.BillBoard.Remove(bed)
						...
						if HasBlocks then
							Utility.Visual.BillBoard.Create(bed)
							for bname, count in Counts do
								local image = BridgeDuel.Constants.Blocks[bname].Image
								if not image then continue end
								Utility.Visual.BillBoard.Add.Image(bed, image)
							end
						end
					end
				end)
			else
				Utility.Misc.Events.Remove('Heartbeat', 'BedOverlay')
				for _, bed in workspace.Map:GetDescendants() do
					if not (bed:IsA('Model') and bed.Name == 'Bed') then continue end
					Utility.Visual.BillBoard.Remove(bed)
				end
...
task.defer(function()
	...
	ESP = Sections.Visual:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Stepped', 'ESP', nil, function()
					for _, v in Players:GetPlayers() do
						if v ~= LocalPlayer and Utility.Entity.IsAlive(v) then
							Utility.Visual.Highlight.Add(v.Character)
						end
					end
					if IncludeBed then
						for _, v in workspace.Map:GetChildren() do
							if v:IsA('Model') and v.Name == 'Bed' then
								if v:GetAttribute('Team') ~= LocalPlayer.Team.Name then
									Utility.Visual.Highlight.Add(v)
								end
							end
						end
					end
				...
				Utility.Misc.Events.Remove('Stepped', 'ESP')
				for _, v in Players:GetPlayers() do
					if v ~= LocalPlayer and Utility.Entity.IsAlive(v) then
						Utility.Visual.Highlight.Remove(v.Character)
					end
				end
				if IncludeBed then
					for _, v in workspace.Map:GetChildren() do
						if v:IsA('Model') and v.Name == 'Bed' then
							Utility.Visual.Highlight.Remove(v)
						end
					end
				end
			end
...
task.defer(function()
	...
	TNTOverlay = Sections.Visual:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Heartbeat', 'TNTOverlay', 0.1, function()
					for _, tnt in workspace.Map:GetChildren() do
						...
						if not Existed[tnt] then Existed[tnt] = {Start = tick()}
							...
							Utility.Visual.BillBoard.Create(tnt)
							Utility.Visual.BillBoard.Add.Text(tnt, '5.0', UDim2.fromOffset(24, 16))
						end
					...
					for i, v in pairs(Existed) do
						if not i or not i.Parent then
							...
							Utility.Visual.BillBoard.Remove(i)
							Existed[i] = nil
						end
					end
				end)
			else
				Utility.Misc.Events.Remove('Heartbeat', 'TNTOverlay')
				for i, v in pairs(Existed) do
					if v.Destruct and v.Blast then
						v.Destruct:Destroy()
						v.Blast:Destroy()
...
task.defer(function()
	...
	AutoSneak = Sections.World:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Heartbeat', 'AutoBridge', nil, function()
					if not Utility.Entity.IsAlive(LocalPlayer) then return end
					if BridgeDuel.Functions.Utility.GetUI() then return end
				...
				Utility.Misc.Events.Remove('Heartbeat', 'AutoBridge')
				BridgeDuel.Knit.GetController('MovementController'):RemoveSpeedOverride('sigeonpex')
				LocalPlayer:SetAttribute('ClientSneaking', false)
			end
		end
...
task.defer(function()
	...
	ProjectileAssist = Sections.World:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Heartbeat', 'ProjectileAssist', nil, function()
					if not Utility.Entity.IsAlive(LocalPlayer) then return end
					if BridgeDuel.Functions.Utility.GetUI() then return end
					if not LocalPlayer.PlayerGui.MainGui:FindFirstChild('FakeCrosshair') then
						Crosshair = LocalPlayer.PlayerGui.MainGui.Crosshair
					...
					local Tool = Utility.Entity.Inventory.Character.Find('bow')
					if not Tool then return end
				...
				Utility.Misc.Events.Remove('Heartbeat', 'ProjectileAssist')
				ChargeTime = nil
				if LocalPlayer.PlayerGui.MainGui:FindFirstChild('FakeCrosshair') then
					Crosshair.Image = FakeCrosshair.Image
					Crosshair.Position = FakeCrosshair.Position
...
task.defer(function()
	...
	Breaker = Sections.World:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				IsBreaking = false
				Utility.Misc.Events.Add('Heartbeat', 'Breaker', nil, function()
					if not Utility.Entity.IsAlive(LocalPlayer) then return end
					if BridgeDuel.Functions.Utility.GetUI() then return end
					...
					if Current and not IsBreaking then
						...
						if Part then
							local Tool = Utility.Entity.Inventory.Character.Find('pickaxe')
							if not Tool then return end
				...
				Utility.Misc.Events.Remove('Heartbeat', 'Breaker')
				IsBreaking = false
				Current = nil
...
task.defer(function()
	...
	AntiStaff = Sections.Misc:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Stepped', 'AntiStaff', 5, function()
					for _, v in Players:GetPlayers() do
				...
				Utility.Misc.Events.Remove('Stepped', 'AntiStaff')
