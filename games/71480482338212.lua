		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Heartbeat', 'AutoClicker', 0, function()
					if not Utility.Entity.IsAlive(LocalPlayer) then BedFight.Functions.Tool.Reset() return end
...
task.defer(function()
	...
	Reach = Sections.Combat:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Stepped', 'Reach', nil, function()
				...
				Utility.Misc.Events.Remove('Stepped', 'Reach')
				for i, v in BedFight.Modules.SwordsData do
					if type(v) == 'table' and v.Range then
						v.Range = 5
					end
...
task.defer(function()
	...
	AutoMLG = Sections.Movement:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Stepped', 'AutoMLG', nil, function()
					if not Utility.Entity.IsAlive(LocalPlayer) then return end
					if BedFight.Functions.Utility.GetUI() then return end
					...
					task.spawn(function()
						while Utility.Entity.IsAlive(LocalPlayer) do
							task.wait()
				...
				Utility.Misc.Events.Remove('Stepped', 'AutoMLG')
				...
				Utility.Misc.Events.Add('Heartbeat', 'BedOverlay', 0.5, function()
					for _, bed in workspace.BedsContainer:GetChildren() do
						...
						for bname, count in Counts do
							...
							Utility.Visual.BillBoard.Add.Image(bed, data.Image)
						end
					end
				end)
			else
...
task.defer(function()
	...
	Breaker = Sections.World:CreateToggle({
		...
		Callback = function(callback)
			if callback then
				Utility.Misc.Events.Add('Heartbeat', 'Breaker', 0.3, function()
					if not Utility.Entity.IsAlive(LocalPlayer) then return end
					if BedFight.Functions.Utility.GetUI() then return end
